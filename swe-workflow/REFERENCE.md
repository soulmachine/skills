...
## Stage 2: `/to-prd` — What does done look like?
...
## Stage 3: `/to-issues` — What are the units of work?
...
## Stage 4: `/triage` — What's actionable?

**Trigger**: issues piling up, ambiguity about what's ready vs blocked.
...
Outputs to issues:
- `ready-for-agent` → AGENT-BRIEF comment (the durable execution contract)
...
3. Auto-detect from project signals:
   ...
   - `.multica/` or `multica.yaml` → `multica`
