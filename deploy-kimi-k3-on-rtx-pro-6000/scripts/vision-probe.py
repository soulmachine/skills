#!/usr/bin/env python3
# Diagnose an empty/failed vision reply: dump the FULL chat message (content + reasoning_content)
# for a solid-red PNG, so a reasoning-parser routing artifact (answer landed in reasoning_content)
# can be told apart from a real MoonViT-3d vision failure (nothing at all). Same probe kimi-k26
# uses, retargeted at llama.cpp's endpoint + this model's alias.
#   python3 vision-probe.py [--base-url http://127.0.0.1:30000] [--model kimi-k3]
...
print("\nVERDICT:", "VISION OK (saw red)" if "red" in got
      else "NO 'red' anywhere — check the server was launched with BUILD_SOURCE=fork and --mmproj "
           "actually resolved (see server startup log for 'mtmd'/projector lines)")
