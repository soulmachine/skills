...
- **Agent (`--server`) stopped and didn't come back** — its `KeepAlive` only restarts on an *unsuccessful* exit (`SuccessfulExit=false`), so a clean exit (code 0) leaves it down until next login/reboot, and capture/input silently stop. If `launchctl print gui/$(id -u)/com.carriez.RustDesk_server` shows `state = not running`, restart it: `launchctl kickstart -k gui/$(id -u)/com.carriez.RustDesk_server`. The root daemon (`service`) has plain `KeepAlive=true` and does not have this issue.
