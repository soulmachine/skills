...
4. **Check the GitHub org name** for every name that survived step 3 — a brand needs the handle as much as the domain, and GitHub is where the code will live:
...
5. **Surface the survivors.** In `.com`-only mode, list one line per free `.com`, in catalog rank order (note each name's category).
   ...
   **Carry the GitHub verdict on the same line as the name**, in both modes — one column, not a second list; the user is picking a brand, and the domain and the handle are one decision. A name where the domain *and* the org are both free is the strongest result the sweep can return: lead with those, and say so. Where a name's domain is free but the org is taken, keep it on the list and mark it — it is still registrable, just with a handle compromise ahead.
...
6. **Prompt the user to register.** Ask which they want to take. Point them to Cloudflare Registrar (or their preferred one) — note that Cloudflare Registrar does not sell every TLD in this set, so a second registrar may be needed. Do not enter payment on their behalf — registration is the user's action.
   ...
   For the matching GitHub org, point them at <https://github.com/organizations/new> — free, no payment, seconds to claim. Worth saying plainly: an org name costs nothing and is **first-come**, so it is the cheapest half of the brand to lose by waiting. Creating it is still the user's action, not yours.
...
- **Live lookups only** — never claim availability from cached knowledge or prior turns of this conversation. Domains and org names get registered between checks.
- **Only an explicit negative means available.** RDAP `404`, WHOIS `DOMAIN NOT FOUND`, or `github.com/<name>` returning `404`. Anything else — `429`, `5xx`, a timeout, an empty body, a `30x` — is `UNKNOWN` or `RESERVED`, reported as such and never as free.
- **Use the bundled scripts rather than hand-rolled `curl`.** `check_domains.sh` for TLDs, `check_github.sh` for the org name. The per-TLD hosts, the `.co` WHOIS special case, the `.app` retry loop, and GitHub's reserved-name redirect are all easy to get wrong in a way that fails silently toward false-available.
...
- **A free domain with a taken org is still a result** — report it, marked, rather than dropping it. The two namespaces are independent and the user may not care equally about both; suppressing half the picture makes that call for them.
- **Never report a GitHub name as available on the strength of an API 404** — see the reserved-name trap above. Web endpoint or nothing.
- **No payment on the user's behalf.** Prompt them to register; they check out. Same for creating the GitHub org, which costs nothing but is still theirs to do.
